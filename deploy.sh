...
# This script uses various CircleCI, Travis CI or GitHub Actions environment
# variables. CircleCI prefix their environment variables with
# `CIRCLE_`, Travis with `TRAVIS_`, and GitHub Actions with `GITHUB_`.
# Documentation:
...
if [ -n "$CIRCLECI" ]; then
	...
	# SRC_DIR="$CIRCLE_WORKING_DIRECTORY"
	SRC_DIR="$PWD"
	...
	REPO_SSH_URL="${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}.git"
else
...
REPO_SSH_URL="${REPO_SSH_URL:-"git@github.com:${REPO_SLUG}"}"
...
if [ -n "$TRAVIS" ] && [ -n "$CIRCLECI" ] && [ -n "$GITHUB_ACTIONS" ]; then
	echo "ERROR: this script requires either CircleCI, Travis CI, or GitHub Actions"
...
if [ -z "${BRANCH}" ]; then
	...
	echo "This variable should be set by Travis CI, CircleCI, and GitHub Actions; if you consistently experience errors please check with WordPress.com VIP support."
	exit 1
fi
